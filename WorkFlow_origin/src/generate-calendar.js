...
function loadAllCharacters() {
  ...
  for (const characterName of folders) {
    // 許可された講師のみを読み込む
    if (!ALLOWED_INSTRUCTORS.includes(characterName)) {
      console.log(`⏭️  ${characterName}をスキップ（許可リストに含まれていません）`);
      continue;
    }
...
- **必ず日本語で記述**
- **【最重要】講師として登場できるのは以下の5人のみです：塾頭高崎翔太、塾長山﨑琢己、CTO井上陽斗、講師鈴木駿介、講師本田樹**
- **【厳守】上記5人以外の講師や人物は絶対に作成・登場させないでください**
- **架空のキャラクターは一切作成しないでください。必ず上記の「キャラクター設定」から選択してください**
...
- **重要: ロゴを使用する場合は、形や色味を変形・変更せず、元のデザインのまま使用すること**
...
- **【絶対厳守】講師として登場できるのは以下の5人のみ：塾頭高崎翔太、塾長山﨑琢己、CTO井上陽斗、講師鈴木駿介、講師本田樹**
- **【絶対厳守】架空のキャラクターや上記5人以外の講師は絶対に作成・登場させないこと**
