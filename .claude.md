...
**if(Tech)** - マインクラフトベースのプログラミング教室
- ホームページ: `index.html`
- Instagram自動化: `WorkFlow_origin/`
...
### 1. homepage-manager
**用途**: ホームページの編集・更新・UI/UX改善
**スキル**: homepage-design
```
例: 「ホームページのFAQセクションを更新して」
```

### 2. instagram-content-creator
**用途**: Instagram投稿コンテンツの作成・カレンダー計画
**スキル**: instagram-content
```
例: 「30日分の投稿カレンダーを作成して」
```
...
### 3. image-generator
**用途**: AI画像生成とテキスト合成
**スキル**: image-generation
**AI**: Gemini API (GitHub Actions)
```
例: 「新しい画像ルールを追加して」
```
...
### 4. character-manager
**用途**: キャラクタープロファイル管理
**スキル**: character-management
```
例: 「新しい講師キャラクターを追加して」
```

### 5. workflow-automator
**用途**: GitHub Actionsワークフロー管理
**スキル**: workflow-automation
```
例: 「ワークフローの状態を確認して」
```
...
### ホームページ関連
- `/homepage/update [section] [content]` - セクション更新
- `/homepage/add-section [name]` - 新規セクション追加

### Instagram関連
- `/instagram/generate-calendar [days]` - カレンダー生成
- `/instagram/create-post [theme]` - 単一投稿作成

### ワークフロー関連
- `/workflow/run-pipeline [mode]` - パイプライン実行
- `/workflow/check-status` - 状態確認

### キャラクター関連
- `/character/add [name]` - 新規キャラクター追加

### 画像関連
- `/image/add-rule [name]` - 画像ルール追加
...
| ワークフロー | 用途 | AI |
|-------------|------|-----|
| content-generation.yml | Instagram投稿生成 | Gemini |
| generate-character.yml | キャラクターCSV生成 | Gemini |
| generate-imagerule.yml | 画像ルール生成 | Gemini |
| claude_bot.yml | Claude Code自動化 | Claude |

**重要**: GitHub ActionsのAI処理はすべて**Gemini API**を使用
